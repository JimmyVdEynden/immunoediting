#########################################
# manuscript_summarize_results_pentaNT.R
#########################################
...
#######
{
  ...
  subst_type_matrix<- readRDS(file="data/subst_type_matrix_ls.rds")$pentaNT
...
##################
{  
  # Prototypical genotype - wt peptides (as HBMR) - COMPLETE ANNOTATIONS (TO CHECK CALCULATIONS ON SUBSET)
  ########################################################################################################
  {
    # Load: same as previous
    # Get HLA binders: same as previous
    # Add subst type 5: same as previous but factorize!
    TCGA_maf$subst_type5<- factor(TCGA_maf$subst_type5)
    ...
    # Exclude driver genes & non-expressed genes
    ...
    Rsim_ls<- calculate_Rsim3(TCGA_maf$Tumor_Sample_Barcode,
                              ...
                              TCGA_maf$isHA,
                              TCGA_maf$subst_type5,
                              exp_sites = subst_type_matrix,
                              exp_mut = subst_type_matrix,
    ...
    results_t_sample_mainWt_complete<- as.data.frame(cbind(sample_t,obs=obs[rownames(sample_t)],exp=exp[rownames(sample_t)],dNHLA_dNnonHLA=Rsim[rownames(sample_t)]))
    results_t_can_mainWt_complete<- as.data.frame(cbind(dNHLA_dNnonHLA_med=Rsim_med[cancers],dNHLA_dNnonHLA_p=Rsim_p[cancers],dNHLA_dNnonHLA_q=Rsim_q[cancers]))
  }
  ...
  #################################################################
  {
    ...
    # Get information on all sites & substitutions
    subst_type_matrix_sites<- readRDS(file="data/subst_type_matrix_ls.rds")$pentaNT
    ...
    subst_type_matrix<- subst_type_matrix$pentaNT_wt
  ...
  ######################################
  {
    ...
    # Get information on  substitutions
    subst_type_matrix<- readRDS(file="data/GPPM_subset_subst_type_matrix_ls.rds")
    subst_type_matrix<- subst_type_matrix$pentaNT_mut
  ...
  #######################################################
  {
    ...
    # Get information on genotypes-specific substitutions
    if(useSimulated_data) subst_type_matrix_ls_perSample<- readRDS(file="data/GPPM_subset_subst_type_matrix5_mut_per_sample_sim.rds")
    else subst_type_matrix_ls_perSample<- readRDS(file="data/GPPM_subset_subst_type_matrix5_mut_per_sample.rds")
...
##############
{
  if(useSimulated_data) save(TCGA_cancer_id,results_t_sample,results_t_can,results_t_sample_mainWt,results_t_can_mainWt,results_t_sample_mainMut,results_t_can_mainMut,results_t_sample_mainWt_complete,results_t_can_mainWt_complete,file="results/data/summary_results_pentaNT_sim.RData")
  else save(TCGA_cancer_id,results_t_sample,results_t_can,results_t_sample_mainWt,results_t_can_mainWt,results_t_sample_mainMut,results_t_can_mainMut,results_t_sample_mainWt_complete,results_t_can_mainWt_complete,file="results/data/summary_results_pentaNT.RData")
}
