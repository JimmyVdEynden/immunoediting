...
########################
{
  ...
  TCGA_maf<- readRDS("data/TCGA_maf_sim_mainHLAType.rds")
  ...
  for(i in 1:10){
    ...
    gene_mhc_rank_temp<- read.table(paste0("temp/wt_out_sim_",i,".xls"),header=T,sep="\t",skip=1,colClasses = c(c("NULL","NULL","NULL"),rep(c("NULL","NULL","NULL","numeric"),length(gene_mhc_colnames)),c("NULL","NULL")))
  ...
  saveRDS(gene_mhc_rank,file = "temp/wt_out_sim_rank.rds")
  ...
  saveRDS(aff_wt_9_ls,file="temp/TCGA_sim_affRank_wt_9.rds")
  ...
  for(i in 1:10){
    ...
    gene_mhc_rank_temp<- read.table(paste0("temp/mut_out_sim_",i,".xls"),header=T,sep="\t",skip=1,colClasses = c(c("NULL","NULL","NULL"),rep(c("NULL","NULL","NULL","numeric"),length(gene_mhc_colnames)),c("NULL","NULL")))
    colnames(gene_mhc_rank_temp)<- gene_mhc_colnames
    if(i==1) gene_mhc_rank<- gene_mhc_rank_temp
    else gene_mhc_rank<- cbind(gene_mhc_rank,gene_mhc_rank_temp)
  }
  ...
  rownames(gene_mhc_rank)<- pep
  saveRDS(gene_mhc_rank,file = "temp/mut_out_sim_rank.rds")
  # Add affinities from wt 
  nonapep_mut_all<- na.omit(unique(unlist(TCGA_maf$mut_pep)))
  gene_mhc_rank_wt<- readRDS(file = "temp/wt_out_sim_rank.rds")
  ...
  saveRDS(aff_mut_9_ls,file="temp/TCGA_sim_affRank_mut_9.rds")
  ...
  # Save final TCGA_maf
  saveRDS(TCGA_maf,file="data/TCGA_maf_sim_ranks.rds")
}